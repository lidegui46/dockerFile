...
rm -rf "${runtimeDirectory}/Dockerfile_7035"
cp "${sourceDirectory}/${appName_dockerFile}/target/code_target.jar" "${runtimeDirectory}/app_runtime.jar"
cp -rf "${sourceDirectory}/${appName_dockerFile}/document/Dockerfile_7035" "${runtimeDirectory}/Dockerfile_7035"

# 访问运行目录 
cd "${runtimeDirectory}"

# 删除原容器
docker rm -f app_container_8090

# 删除原镜像
# 此方法会添加很多 none 空镜像
# docker rmi app_image &> /dev/null
docker rmi app_image_7035
echo -e "rmi old image done...................."

# 构建新镜像
echo -e "build new image starting.............."
docker build -f ./Dockerfile_7035 -t app_image_7035 .
echo -e "build new image done.................."

# 启动容器
docker run -d -p 8090:8090 --name app_container_8090 app_image_7035
